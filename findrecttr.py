...
#Сетевые параметры
IP_SERVER = '192.168.8.170' #IP адрес сервера
...
debugClient = xmlrpc.client.ServerProxy('http://%s:%d' % (IP_SERVER, DEBUG_PORT))
...
while True:
    ...
    if ret:
        ...
        if res:
            debugClient.debugFrame(('xXx_Hacker_xXx', imgJpg.tobytes()))#заслал картинку  

        cv2.imshow('xXx_Hacker_xXx', frame) #Отображаем кадр
